...
class Logger:
    ...
    def __init__(self, log_file, l_offset=2, r_offset=2):
        f = open(log_file, 'r')
    ...
    def print_error(self):
        for (i, error) in enumerate(self.errors):
            print
            print '='*80
            print "#%d Failed assertion at line %s, file %s: " % (i + 1,
                                                                  error.line -
                                                                  1, error.file)
            print
            print "   Expected:  %s " % ( error.msg)
            print '='*80
...
Expand error log for specified test code."""
    ...
    parser.add_option("-n", action="store", dest="l_offset",type="int",
                        default=2, help="Set test code region")
